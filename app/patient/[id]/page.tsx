...
function Row({ label, value, highlight }: { label: string; value: React.ReactNode; highlight?: 'red' | 'yellow' | 'green' }) {
  ...
  return (
    <div className="flex items-start gap-2 text-sm py-1 border-b border-gray-100 last:border-0">
      ...
      <span className={`text-gray-900 font-medium flex-1 ${bg}`}>{value}</span>
    </div>
  )
}
    ...
    <main className="max-w-6xl mx-auto p-4 pb-16">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="bg-white/20 border border-white/30 text-white text-sm font-semibold px-3 py-1 rounded-full">
                {patient.room}
              </span>
            </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 min-w-[180px]">
          ...
          {patient.careTeam.consults.map((c, i) => (
            <div key={i} className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 min-w-[180px]">
      ...
      {/* ── PMH ── */}
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-500 mb-3">📋 Past Medical History</h3>
        <div className="flex flex-wrap gap-2">
          {patient.pmh.map((item, i) => (
            <span key={i} className="bg-gray-100 text-gray-800 text-sm px-3 py-1 rounded-full border border-gray-200">{item}</span>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <Section title="🫁 Respiratory">
          {patient.respiratory.intubationDate ? (
            <>
              ...
              {patient.respiratory.lastAbg && <Row label="Last ABG" value={patient.respiratory.lastAbg} />}
            </>
          ) : (
            <>
        ...
        <Section title="❤️ Cardiac">
          ...
          <Row
            ...
            highlight={patient.cardiac.bpTrend === 'worsening' ? 'red' : undefined}
          />
        ...
        <Section title="🍽️ GI / GU / Nutrition">
          ...
          />
          <Row label="Urine Output" value={patient.nutrition.urineOutput} highlight={patient.nutrition.urineOutput.toLowerCase().includes('low') ? 'red' : undefined} />
        ...
        <Section title="🩹 Skin">
          {patient.skin.length === 0 && !patient.accessSite ? (
            ...
            <>
              {patient.accessSite && (
                <Row label="Access Site" value={`${patient.accessSite.location} — ${patient.accessSite.notes} (${patient.accessSite.date})`} highlight="yellow" />
              )}
        ...
        <Section title="💊 Active Drips">
          {patient.drips.length === 0
            ? <div className="text-sm text-gray-400">No active drips</div>
            : patient.drips.map((d, i) => (
                <div key={i} className="py-1.5 border-b border-gray-100 last:border-0">
                  ...
                  <div className="text-sm text-gray-500">{d.concentration} @ {d.rate}</div>
                </div>
              ))
          }
        </Section>
          ...
          {patient.labs.map((l, i) => (
            <div key={i} className={`p-2 rounded-lg border text-sm ${l.abnormal ? 'border-red-200 bg-red-50' : 'border-gray-100 bg-gray-50'}`}>
              ...
              <div className={`font-bold text-base ${l.abnormal ? 'text-red-700' : 'text-gray-900'}`}>
                {l.value} <span className="text-sm font-normal">{l.unit}</span> {l.trend}
              </div>
            </div>
          ))}
        </div>
